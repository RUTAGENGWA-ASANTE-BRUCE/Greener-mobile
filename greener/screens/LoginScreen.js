...
export default function LoginScreen() {
  ...
  return (
    ...
    <TextInput style={tw` w-full border-b-2 border-gray-200 h-12 p-1 mt-3 rounded-md `}/>

    <Text style={tw`text-gray-400 mt-5`}>Password</Text>
    <TextInput style={tw` w-full border-b-2 border-gray-200 h-12 p-1 mt-3 rounded-md `} secure={secure}/>
  {/* <Icon style={{ paddingRight: 15, }} name={secure ? "eye" : 'eye-slash'}size={20} color='gray' onPress={() => setSecure(!secure)} /> */}
  <TouchableOpacity style={tw`mt-8  rounded-lg `} onPress={()=>navigator.navigate("BrowseScreen")}>
        <LinearGradient colors={['#09db9f', '#48e0b5', '#1bcc9a']} start={[0, 1]} end={[1, 0]} style={tw`rounded-md w-full py-2.5 `}>
